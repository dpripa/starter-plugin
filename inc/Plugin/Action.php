...
class Action {
	...
	protected static function add( string $type, string $name, callable $callback ): void {
		add_action( static::TYPES[ $type ] . '_' . KEY . '_' . $name, $callback );
		add_action( static::TYPES[ $type ] . '_nopriv_' . KEY . '_' . $name, $callback );
	}
	...
	protected static function get_url( string $type, string $name = '' ): string {
		...
		if ( $name ) {
			if ( ! has_action( static::TYPES[ $type ] . '_' . KEY . '_' . $name ) ) {
				throw new \Exception( "The \"$name\" action isn't defined" );
			}

			return add_query_arg( array( 'action' => KEY . '_' . $name ), $url );
		}
